...
You are the lead engineer and product designer for **CareFlow** - real-time patient-flow management for hospitals - digital tokens, live queues, bed and ICU visibility and AI wait-time prediction.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8110**. Frontend dev server: **5110**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11100-11109**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - CareFlow (the latest, advanced, innovative system)
...
- **F2 Live queues** - OPD, laboratory and pharmacy queues updated in real time (WebSockets), with a TV display mode.
- **F3 Patient journey tracking** - state machine across departments; timeline per patient.
- **F4 Laboratory scheduling** - test slots allocated by current load.
- **F5 Bed and ICU board** - real-time availability; allocation suggestions.
- **F6 AI wait-time prediction** - model trained on the Kaggle patient-flow and ER wait-time data, shown on tokens and screens.
- **F7 Emergency priority** - triage score moves critical patients up the queue.
- **F8 Role dashboards and analytics** (NEW) - doctor, nurse, lab, admin views; throughput and wait-time analytics.
- **F9 Patient notifications** (NEW) - live token status page plus optional free Telegram alerts.
...
- Wait-time regression (gradient boosting) and triage priority classification trained locally; report MAE / RMSE and accuracy.
- A seeded simulator generates a realistic day of arrivals so queues move live in the demo.
...
**Specific to CareFlow**
- FastAPI WebSockets, qrcode
...
2. Optional: Telegram bot token for patient alerts (guided).
...
- [ ] It runs on its own ports (8110 / 5110) while other products run on the same PC.
