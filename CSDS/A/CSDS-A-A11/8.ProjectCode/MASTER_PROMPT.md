...
You are the lead engineer and product designer for **ShopTwin** - an adaptive e-commerce recommendation engine that tests strategies in a digital twin before customers ever see them.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8111**. Frontend dev server: **5111**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11110-11119**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - ShopTwin (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Demo storefront** - catalogue built from the REES46 sample; tracks views, cart and purchases.
- **F2 Behaviour analytics pipeline** - batch jobs aggregating engagement signals (pandas).
- **F3 Multi-agent recommender** - collaborative (ALS), content-based, trending and session-based agents plus an orchestrator that blends them.
- **F4 Digital twin** - a simulated customer population replays behaviour to evaluate strategies offline (CTR, conversion, revenue) before deployment.
- **F5 Explainable recommendations** - 'because you viewed...', 'popular with similar shoppers', attribute matches.
- **F6 Self-learning feedback** - models update incrementally from new events.
- **F7 Traffic optimisation** - response caching, priority queue and graceful degradation, demonstrated with a load test (Locust) (NEW).
- **F8 Admin analytics** (NEW) - strategy comparison, twin results, live metrics.
...
1. Landing page
2. Login / register
3. Storefront (home, product page, cart)
4. 'Why this?' explanation
5. Admin: strategies and digital-twin runs
6. Admin: analytics
7. Admin: load-test results
...
- ALS (implicit), content similarity, session co-occurrence models trained locally on the committed sample; report precision@k, recall@k, NDCG, coverage.
- Digital-twin simulation calibrated from real event statistics.
...
**Specific to ShopTwin**
- implicit (ALS), scikit-learn, SHAP / feature-based explanations, Locust (load tests), cachetools
...
- [ ] It runs on its own ports (8111 / 5111) while other products run on the same PC.
