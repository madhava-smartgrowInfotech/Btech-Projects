...
You are the lead engineer and product designer for **PlastiScan** - AI detection and localisation of plastic waste in ocean and underwater imagery, from photos to video surveys.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8114**. Frontend dev server: **5114**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11140-11149**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - PlastiScan (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Image detection** - upload a photo; YOLO detects plastic and debris classes with boxes and confidence.
- **F2 Underwater enhancement** (NEW) - optional colour correction / CLAHE before detection, with a before-and-after view.
- **F3 Small-debris mode** (NEW) - SAHI sliced inference for large or high-resolution images.
- **F4 Video survey** (NEW) - upload a video; tracking counts unique items per class; annotated video output.
- **F5 Survey reports** (NEW) - per-class counts, density, location tag on a map, PDF export.
- **F6 Clean-up path suggestion** (NEW) - an A*-based collection path over the detected debris positions (inspired by 4.1).
- **F7 Model performance** - mAP50, mAP50-95, precision, recall and per-class results.
...
1. Landing page
2. Login / register
3. Analyse image (detections, enhancement toggle, SAHI)
4. Video survey (progress, annotated video, counts)
5. Survey history and map
6. Report view / PDF
7. Model performance
...
- Train YOLO (Ultralytics) on Kaggle GPU; the product loads the trained weights on CPU.
- Report mAP50, mAP50-95, precision, recall and per-class AP; keep training curves.
...
**Specific to PlastiScan**
- Ultralytics YOLO, SAHI, supervision (ByteTrack tracking and annotation), OpenCV, Leaflet, ReportLab
...
1. Upload an underwater photo -> plastic items boxed with confidence; toggle enhancement and see more detections.
2. Upload a short survey video -> annotated output and unique item counts per class.
3. Save the survey with a location -> map pin and PDF report with a suggested clean-up path.
...
- [ ] It runs on its own ports (8114 / 5114) while other products run on the same PC.
