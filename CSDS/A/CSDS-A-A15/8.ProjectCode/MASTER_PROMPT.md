...
You are the lead engineer and product designer for **VisionForge AI** - explainable visual inspection for manufacturing - detects defects, shows why, suggests root causes and writes the inspection report.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8115**. Frontend dev server: **5115**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11150-11159**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - VisionForge AI (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Defect detection** - YOLO models for steel-surface defects (NEU, 6 classes) and PCB defects (6 classes) with boxes and confidence.
- **F2 Unseen-defect anomaly mode** (NEW) - feature-distance anomaly scoring for defects not in the training classes.
- **F3 Visual explanation** - Grad-CAM / EigenCAM heatmap overlays.
- **F4 Vision-language root cause** - Gemini (multimodal) explains the defect in plain language and suggests likely root causes and corrective actions, grounded in a defect knowledge base you write.
- **F5 Inspection report** - PDF with defect type, confidence, heatmap, explanation, causes and actions.
- **F6 Batch / line inspection** (NEW) - inspect a folder of images; pass/fail summary.
- **F7 Defect analytics** (NEW) - history, Pareto chart of defect types, trends.
...
- Train YOLO (Ultralytics) on NEU and PCB data on Kaggle GPU; build the anomaly model from normal samples; inference on CPU.
- Report mAP50, precision, recall per class; anomaly AUROC.
...
**Specific to VisionForge AI**
- Ultralytics YOLO, pytorch-grad-cam, OpenCV, ReportLab
...
3. Optional: register for MVTec AD (guided).
...
1. Upload a steel image -> 'scratches' detected with a heatmap, explanation, likely causes and actions.
2. Upload a PCB image -> 'missing hole' detected; download the PDF inspection report.
3. Run batch inspection on a folder -> pass/fail summary; analytics show the Pareto of defect types.
...
- [ ] It runs on its own ports (8115 / 5115) while other products run on the same PC.
