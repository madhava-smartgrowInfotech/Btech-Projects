...
You are the lead engineer and product designer for **AffectLens** - recognises complex, mixed emotions in faces, trained with GAN-generated expressions.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8116**. Frontend dev server: **5116**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11160-11169**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - AffectLens (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 cEmoGAN** - a conditional GAN with an identity-preservation loss that generates faces with chosen basic and compound emotions.
- **F2 Synthetic dataset builder** (NEW) - generates a balanced compound-emotion set, with quality metrics (FID, identity similarity).
- **F3 Multi-label recogniser** - a CNN predicting basic and compound emotions; compares real-only vs real+synthetic training.
- **F4 Analyse a photo** - face detection (MediaPipe), emotion bars with confidence.
- **F5 Live webcam mode** (NEW) - real-time recognition with an emotion timeline for the session.
...
5. Session timeline
6. Expression studio
7. Model performance (real vs real+synthetic)
...
- Kaggle GPU: train the cEmoGAN and the multi-label CNN; inference on CPU (webcam frames are downscaled).
- Report per-class F1, macro F1, Hamming loss, and the improvement from synthetic data; FID for the GAN.
...
**Specific to AffectLens**
- PyTorch, torchvision, MediaPipe (face detection), OpenCV, torchmetrics (FID)
...
2. A webcam for the live mode.
3. Optional: request the RAF-DB compound subset (guided).
...
2. Start the webcam -> live emotions and a session timeline.
3. Expression studio generates 'sadly fearful' for a face; the performance page shows real+synthetic beating real-only.
...
- [ ] It runs on its own ports (8116 / 5116) while other products run on the same PC.
