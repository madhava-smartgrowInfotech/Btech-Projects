...
You are the lead engineer and product designer for **CropSight** - smart crop delivery - AI quality grading, price intelligence, storage monitoring and a farmer-to-buyer marketplace.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8117**. Frontend dev server: **5117**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11170-11179**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - CropSight (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Quality grading** - photo -> fresh/rotten and grade A/B/C with defect percentage (CNN), with a Grad-CAM view.
- **F2 Price forecasting** - per commodity and market, from mandi price history (XGBoost with lag features); 'best time to sell' suggestion.
- **F3 Storage and transport monitoring** - live temperature and humidity from an ESP32 + DHT22 (firmware included) **or** the built-in simulator; spoilage-risk alerts.
- **F4 Explainability** - Grad-CAM for grading, SHAP for prices.
- **F5 Marketplace** (NEW) - farmers list lots with grade and suggested price; buyers browse and make offers.
- **F6 Delivery recommendation** (NEW) - nearest suitable buyers or markets and a route on the map (OSRM).
- **F7 Weather** - OpenWeatherMap forecast in pricing and storage advice.
...
3. Grade my produce
4. Price outlook
5. Storage monitor (live sensors)
6. Marketplace (list lot, browse, offers)
7. Delivery planner (map)
8. Admin analytics
...
- Kaggle GPU: CNN grading model; price models trained locally (XGBoost) with MAE / MAPE; grading accuracy, F1 and confusion matrix.
...
**Specific to CropSight**
- PyTorch (CNN), XGBoost, SHAP, Leaflet + OSRM public routing, OpenWeatherMap API
- ESP32 (Arduino) firmware in `firmware/` sending readings to the API over Wi-Fi, plus a simulator for when no board is connected
...
2. OpenWeatherMap API key (free) - https://openweathermap.org/api
3. Do you have an ESP32 + DHT22? If yes, guide the flashing with the Arduino IDE; if not, use the simulator.
4. Optional: data.gov.in API key for live mandi prices.
...
- [ ] It runs on its own ports (8117 / 5117) while other products run on the same PC.
