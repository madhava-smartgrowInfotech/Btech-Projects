# MASTER PROMPT - Attendance Magic
...
You are the lead engineer and product designer for **Attendance Magic** - proxy-proof attendance: geo-fenced sessions, a live face challenge and duplicate-face blocking, all from a phone browser.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8102**. Frontend dev server: **5102**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11020-11029**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - Attendance Magic (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Session creation** - host creates a time-bound session, draws a geo-fence on a map (centre + radius), gets a unique link and QR code.
- **F2 Secure join** - attendee signs in (JWT); device binding and roll/ID restriction per session.
- **F3 Location check** - GPS position must be inside the geo-fence with acceptable accuracy; basic mock-location heuristics (NEW).
- **F4 Live facial challenge** - a random challenge (turn left/right, blink, smile) verified in real time with MediaPipe landmarks.
- **F5 Face verification and duplicate block** - InsightFace embedding matched to the attendee's enrolled face; the same face cannot mark attendance for another identity in the same session.
- **F6 Face enrolment with quality checks** (NEW) - blur, lighting and pose checks before saving the embedding.
- **F7 Live host dashboard** (NEW) - WebSocket updates: count, map pins, flagged attempts.
- **F8 Reports** - per-session lists, summaries, Excel export.
- **F9 Suspicious-attempt log** (NEW) - every rejected attempt with its reason (outside fence, failed challenge, face mismatch, duplicate face).
...
2. Login / register
3. Host dashboard
4. Create session (map geo-fence picker, QR)
5. Live session monitor
6. Attendee join flow: location -> live challenge -> verification -> success
7. Face enrolment
8. Reports and Excel export
9. Admin: people and roll lists
...
- No training required: MediaPipe Face Landmarker (in the browser) for challenges; InsightFace (`buffalo_l` via onnxruntime, CPU) for embeddings and verification.
- Calibrate the verification threshold with a small consented test set (volunteer photos) and report false-accept / false-reject rates in the product and docs.
...
**Specific to Attendance Magic**
- MediaPipe Tasks (web), InsightFace + onnxruntime, Leaflet (geo-fence map), openpyxl (Excel export), qrcode
...
1. 2-3 volunteers with Android/iPhone phones (Chrome/Safari) for testing.
2. Install `cloudflared` (guided) for the HTTPS phone link.
3. Default geo-fence radius and session length (defaults: 100 m, 10 minutes).
...
1. Host logs in, creates a session with a 100 m geo-fence around the current building -> QR and link shown.
2. Attendee opens the HTTPS link on a phone -> location accepted -> 'turn your head left' challenge -> face verified -> marked present; the host dashboard updates live.
3. A second account tries with the same face -> blocked as duplicate and logged.
4. Host ends the session -> summary and Excel export.
...
- [ ] It runs on its own ports (8102 / 5102) while other products run on the same PC.
