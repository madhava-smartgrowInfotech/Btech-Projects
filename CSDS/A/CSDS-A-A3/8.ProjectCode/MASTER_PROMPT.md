...
You are the lead engineer and product designer for **PhishShield** - real-time phishing URL detection that combines machine learning with continuously updated threat intelligence.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8103**. Frontend dev server: **5103**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11030-11039**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - PhishShield (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 URL check** - single URL and bulk CSV; verdict, probability and risk level.
- **F2 Feature engine** - lexical, structural and domain features (length, entropy, special characters, subdomains, TLD, IP use, domain age via WHOIS where available).
- **F3 Model comparison** - Random Forest, XGBoost and Logistic Regression trained and compared; best model served; metrics page.
- **F4 Threat-intelligence sync** - pulls the free OpenPhish community feed (and PhishTank if a key is available), caches it, and gives an instant match before the model runs.
- **F5 Crowdsourced reporting loop** (NEW) - users report URLs; a moderator verifies; verified reports feed the next retraining.
- **F6 Why flagged** (NEW) - SHAP explanation for each verdict.
- **F7 One-click retraining with model versions** (NEW) - retrain, compare with the current model, promote or roll back; drift chart.
- **F8 Browser extension** (NEW) - Chrome Manifest V3 extension that calls the local API and warns before visiting risky links.
- **F9 Analytics** (NEW) - trends, top risky TLDs, report volumes.
...
6. Reports and moderation queue
7. Models (metrics, versions, retrain)
...
- Train locally on CPU: Random Forest, XGBoost and Logistic Regression on the Kaggle 'Phishing Site URLs' dataset (about 549k labelled URLs); stratified split; report accuracy, precision, recall, F1, ROC-AUC; SHAP for explanations.
- Retraining merges verified crowdsourced reports into the training data and records a new model version.
...
**Specific to PhishShield**
- XGBoost, SHAP, tldextract, python-whois
- OpenPhish community feed: https://openphish.com/feed.txt (free, no key)
...
1. Kaggle account and API token (for the download script).
2. Optional: PhishTank API key (the product works without it).
3. Chrome browser to load the unpacked extension (guided).
...
1. Check a known phishing URL -> 'High risk' with SHAP reasons; check a well-known safe site -> 'Safe'.
2. Upload a CSV of 50 URLs -> bulk results and a downloadable report.
3. Report a new suspicious URL -> moderator approves -> retrain -> new model version appears with metrics.
4. Load the Chrome extension and visit a flagged URL -> warning page shown.
...
- [ ] It runs on its own ports (8103 / 5103) while other products run on the same PC.
