...
You are the lead engineer and product designer for **HubOptima** - predict-then-optimise planning of e-commerce logistics hubs, balancing cost and delivery speed.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8104**. Frontend dev server: **5104**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11040-11049**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - HubOptima (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Data import and preparation** - load the Olist orders, customers, sellers and geolocation data; aggregate demand by region.
- **F2 Demand forecasting** - Random Forest per-region demand forecast, with quantile bands (NEW).
- **F3 Hub location optimisation** - capacitated p-median / hub-location model solved with OR-Tools (MIP) plus a matheuristic (fix-and-optimise / neighbourhood search) for speed.
- **F4 Constraints editor** - hub capacity, fixed opening cost, maximum service distance / SLA, number of hubs.
- **F5 Interactive map** - hubs, customer regions, allocations and flows on Leaflet.
- **F6 What-if scenarios** (NEW) - demand +20%, hub closure, capacity change; compare scenarios side by side.
- **F7 KPIs vs baseline** - total cost, average distance/time, SLA compliance, hub utilisation compared with a greedy baseline.
- **F8 Report export** (NEW) - PDF / Excel of the chosen plan.
...
2. Login / register
3. Data overview
4. Demand forecast (charts, bands)
5. Optimisation setup (constraints)
6. Results map + KPIs
7. Scenarios and comparison
8. Reports
...
- Random Forest regression (scikit-learn) and a quantile gradient-boosting model, trained locally; report MAE, RMSE, MAPE.
- Optimisation: OR-Tools (CP-SAT or CBC MIP) and a matheuristic; report objective value, run time and optimality gap.
...
**Specific to HubOptima**
- Google OR-Tools (or PuLP + CBC), Leaflet + react-leaflet, geopy (distances), ReportLab / openpyxl
...
1. Open the forecast page -> demand per region with uncertainty bands.
2. Set 5 hubs, capacity and a 500 km service limit -> optimise -> hubs and allocations on the map with KPIs vs baseline.
3. Run the 'demand +20%' scenario -> compare plans side by side -> export the report.
...
- [ ] It runs on its own ports (8104 / 5104) while other products run on the same PC.
