...
You are the lead engineer and product designer for **TaxSentinel** - self-supervised detection and plain-language explanation of GST input-tax-credit fraud patterns.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8105**. Frontend dev server: **5105**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **11050-11059**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - TaxSentinel (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 GST ecosystem generator** - seeded, realistic synthetic data: taxpayers, invoices, monthly returns, with injected fraud (fake invoices, circular trading, shell entities, ITC spikes, filing mismatches) and ground-truth labels.
- **F2 Feature layers** - invoice-level, taxpayer-behaviour and network-level features.
- **F3 JEPA behaviour encoder** - self-supervised model that learns normal behaviour; anomaly score = embedding-prediction error.
- **F4 Fraud-ring analytics** - circular-trading cycle detection and shell-cluster detection (NetworkX) with an interactive graph view.
- **F5 Risk scoring and ranking** - combined score per taxpayer and invoice chain with drill-down.
- **F6 Tuned LLM explanations** - a small open LLM fine-tuned with LoRA writes an investigator-style narrative citing the evidence; Gemini as fallback.
- **F7 Case management** (NEW) - assign, investigate, record outcome; outcomes become labels.
- **F8 Evaluation dashboard** (NEW) - precision@k, recall, F1 against the injected fraud labels; comparison with a rule-based baseline.
...
4. Taxpayers (risk ranking, profile)
5. Fraud-ring graph explorer
6. Case detail with LLM narrative
7. Case management
8. Model performance
...
- JEPA encoder (PyTorch) trained on Kaggle GPU; small LLM (for example `Qwen/Qwen2.5-0.5B-Instruct`) fine-tuned with LoRA (PEFT) on Kaggle GPU to generate explanations; both run on CPU for inference.
- Rule-based baseline for comparison; report precision@k, recall, F1, PR curve.
- PaySim transactions can be used to pre-train the behaviour encoder on real-looking payment sequences.
...
**Specific to TaxSentinel**
- PyTorch, NetworkX, Hugging Face Transformers + PEFT (LoRA), react-force-graph (graph view)
...
2. Gemini API key (fallback explanations) - https://aistudio.google.com/apikey
3. Review the fraud patterns and invoice fields you propose for the generator (show them in Phase 2).
...
1. Dashboard shows total ITC claimed, flagged taxpayers and top risk scores.
2. Open the top-ranked taxpayer -> evidence (ITC spike, mismatched returns) and the fraud-ring graph showing a circular-trading loop.
3. Open the case -> LLM narrative citing the evidence -> mark 'confirmed fraud'.
4. Model performance page shows precision@k and recall vs the rule baseline.
...
- [ ] It runs on its own ports (8105 / 5105) while other products run on the same PC.
