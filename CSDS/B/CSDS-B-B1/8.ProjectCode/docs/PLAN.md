...
**Data:** CivicComp-HiEn (16k real Bengaluru complaints x EN/HI/Hinglish, severity labels; annotated CSV committed gzipped, 5 MB - the 1.1 GB of model weights skipped) + Citizen Grievance Dataset (3.7k Hindi/Hinglish/EN, committed) + Indian Citizen Complaint (2k EN, committed) -> 15 unified categories -> 8 departments. NYC 311 extract (~300k closed requests, created/closed dates, gzipped) -> resolution-time model. Seeded generator -> wards (Bengaluru sample city), officers, 700 labelled sample complaint histories. Kaggle downloads work anonymously (no token needed).

**ML (CPU, < 1 min each):** category + department = TF-IDF (word 1-2 + char 2-4) + LogisticRegression; priority = XGBoost on text-urgency probability + multilingual urgency cues + category (Low/Med/High from CivicComp severity, Critical = top severity score); resolution days = XGBoost median regressor on NYC 311 (category, priority, weekday, hour). Metrics -> `experiments/*.json`.
