...
You are the lead engineer and product designer for **APISentry** - automated security testing for payment APIs - OWASP API Top 10 checks, AI-generated business-logic tests, a security score and clear fixes before deployment.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8213**. Frontend dev server: **5213**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **12130-12139**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - APISentry (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Target import** - OpenAPI / Swagger (JSON / YAML), Postman collection or manual endpoints; auth profiles (bearer JWT, API key, basic) including two users for access-control tests.
- **F2 Scope guard** - scans run only against allow-listed hosts (localhost by default) after the user confirms they own or are authorised to test the target; the scanner rate-limits itself.
- **F3 OWASP API Top 10 tests** - BOLA / IDOR, broken authentication (missing, invalid, expired tokens, weak JWT algorithms and secrets), object-property authorization and excessive data exposure / mass assignment, unrestricted resource consumption (rate limiting), function-level authorization, injection payloads, security misconfiguration (CORS, headers, verbose errors, methods), improper inventory (undocumented and old endpoints), basic SSRF probes.
- **F4 AI test generation** (NEW) - Gemini reads the spec and proposes business-logic tests (negative amounts, changed account IDs, replayed payment IDs); results feed back into follow-up tests.
- **F5 Payment-specific checks** (NEW) - amount tampering, currency mismatch, idempotency-key replay, double-spend race, webhook signature validation.
- **F6 Security score** - 0-100 with grade and severity weighting; per-endpoint heatmap.
- **F7 Findings** - evidence (request / response), curl reproduction, OWASP mapping, fix recommendations with code examples.
- **F8 Reports and CI mode** - PDF / HTML reports; JUnit and SARIF export; command-line `apisentry scan --spec ... --fail-on high` (NEW).
- **F9 Scan history and diff** (NEW) - fixed, new and recurring findings.
- **F10 Built-in test targets** - VAmPI plus 'DemoPay', a deliberately vulnerable payments API shipped in `targets/`; a validation page compares findings with each target's known vulnerability list.
...
1. Landing page
2. Login / register
3. Projects and targets (import, auth profiles, scope)
4. Scan configuration and live progress
5. Results (score, heatmap, findings)
6. Finding detail (evidence, fix)
7. Reports
8. Scan history and diff
9. Validation page
...
- No model training. Gemini generates and explains tests. Evaluate detection rate and false positives against the known vulnerabilities of VAmPI and DemoPay.
...
**Specific to APISentry**
- httpx (async), prance / openapi-spec-validator (spec parsing), PyJWT (token tests), Typer (CLI), Jinja2 + ReportLab (reports)
- VAmPI (runs with plain Python) - https://github.com/erev0s/VAmPI ; OWASP crAPI optional via Docker - https://github.com/OWASP/crAPI
- Replaces the original plan's Flask / Streamlit dashboard with React + FastAPI.
...
2. Confirm scans stay local (default: localhost only).
3. Optional: Docker Desktop if you also want crAPI.
...
1. Import DemoPay's OpenAPI spec with two user tokens -> scan runs -> score 38/100 (grade D).
2. Findings: BOLA on /accounts/{id}, amount tampering on /payments, missing rate limit on /login - each with evidence and a fix.
3. Run the CLI with --fail-on high -> non-zero exit code, JUnit file produced.
4. Fix one issue in DemoPay, rescan -> the diff shows it as fixed.
...
- [ ] It runs on its own ports (8213 / 5213) while other products run on the same PC.
