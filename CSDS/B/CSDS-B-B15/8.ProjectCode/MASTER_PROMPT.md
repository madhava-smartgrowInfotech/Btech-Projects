...
You are the lead engineer and product designer for **MediQueue** - district-wide outpatient booking that prioritises by severity, spreads patients across hospitals and shows a live queue position with waiting time.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8215**. Frontend dev server: **5215**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **12150-12159**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - MediQueue (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 District hospital network** - Hyderabad hospitals from OpenStreetMap (name, location, type) with configured specialties, daily OP limits and emergency quotas (seeded, editable).
- **F2 Patient booking** - symptoms (guided picker or free text mapped by Gemini), location and date -> severity -> ranked hospital suggestions (severity fit, distance, specialty, availability, predicted wait) -> booking with token.
- **F3 Severity AI** - mild / moderate / severe / critical from a symptom model, symptom-severity weights and red-flag rules; critical cases get emergency advice and use the emergency quota.
- **F4 Limits, quotas and next-day scheduling** - automatic when a day is full; ILP slot optimisation with no-show prediction and controlled overbooking (NEW).
- **F5 Live queue position** - position and estimated wait update in real time (WebSocket) as doctors call patients or emergencies are inserted.
- **F6 Hospital console** - today's queue, call next, no-show, add emergency, reschedule.
- **F7 Referrals** - a doctor refers the patient to another hospital or specialty with a consented summary; status tracking.
- **F8 Notifications** - in-app plus free Telegram / email.
- **F9 District dashboard** (NEW) - load across hospitals, waiting times, severity mix, map.
...
1. Landing page
2. Login (patient, hospital staff, doctor, district admin)
3. Book a visit (symptoms, severity, hospital suggestions on map)
4. My token (live position and wait)
5. Hospital console
6. Referrals
7. District dashboard
8. Admin: hospitals, specialties, quotas
9. Model performance
...
- Local CPU: disease prediction from symptoms (feeds severity), no-show predictor, waiting-time estimator (queue model plus regression on simulated service times).
- Report accuracy / macro-F1, no-show ROC-AUC, waiting-time MAE; the severity rules are documented.
...
**Specific to MediQueue**
- Leaflet + OpenStreetMap, Overpass API (one-time hospital fetch, cached) - https://overpass-api.de/ , OR-Tools or PuLP (slot ILP), FastAPI WebSockets
- Cloudflare quick tunnel for HTTPS on phones (section 10, Phase 2)
- Replaces the original plan's Google Maps API, Django / Node and PostgreSQL / MongoDB with Leaflet + OpenStreetMap, FastAPI and SQLite.
...
2. Gemini API key (free-text symptoms) - https://aistudio.google.com/apikey
3. An Android phone for the patient flow; install `cloudflared` (guided).
4. Optional: Telegram bot token for notifications.
...
- [ ] It runs on its own ports (8215 / 5215) while other products run on the same PC.
