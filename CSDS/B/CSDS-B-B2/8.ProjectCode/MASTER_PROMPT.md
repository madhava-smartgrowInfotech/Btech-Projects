...
You are the lead engineer and product designer for **SignalScout** - finds weak and dead mobile-network zones automatically from real phone and sensor readings, points users to better signal and files complaints with technical evidence.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8202**. Frontend dev server: **5202**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **12020-12029**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - SignalScout (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Android field probe** - Kotlin app reads real cellular metrics (RSSI, RSRP, RSRQ, SINR, network type, operator, cell ID) and GPS on a schedule; logs in the background; stores offline and syncs later.
- **F2 ESP32 sensor node** - fixed node reporting Wi-Fi / BLE RSSI and GPS (NEO-6M) to the API; a simulator replaces it when there is no hardware.
- **F3 Zone classification** - Strong / Weak / Dead for every reading and map cell with an ML model and confidence.
- **F4 Coverage heatmap** - Leaflet heat and hex-grid layers filtered by operator, network type and time of day.
- **F5 Better-signal suggestions** - Gaussian Process model predicts signal around the user and suggests the nearest spot with strong signal.
- **F6 Automatic complaints** - a zone that stays Weak/Dead beyond a threshold creates a complaint with evidence (metrics, location, time, cell ID); lifecycle Detected -> Registered -> Acknowledged -> In progress -> Resolved.
- **F7 Auto-verification** (NEW) - a resolved complaint is confirmed or reopened automatically from new readings in that zone.
- **F8 Operator desk** - complaint queue, map, evidence and status updates for engineers; email / Telegram notifications (free).
- **F9 Offline-first** - the app and sync work through power or internet outages.
- **F10 Analytics** (NEW) - dead-zone trends, worst areas, time-of-day patterns, operator comparison.
...
1. Landing page
2. Login (user, engineer, admin)
3. Android app: live meter, readings log, sync status, nearest better-signal spot
4. Coverage map (heat, hex grid, filters)
5. My complaints
6. Operator desk (queue, complaint detail with evidence)
7. Devices (phones, ESP32 nodes)
8. Analytics
9. Model performance
...
- Local CPU: zone classifier (Random Forest / XGBoost) and Gaussian Process regressor trained on public signal measurements, then validated on the user's own collected readings.
- Labels follow documented 3GPP-style signal ranges; report accuracy, macro-F1, confusion matrix, and interpolation RMSE on held-out points.
...
**Specific to SignalScout**
- Android app: Kotlin + Jetpack Compose + Room + WorkManager + Retrofit, in `android/`
- ESP32 firmware (Arduino C++) in `firmware/`
- Leaflet + leaflet.heat, scikit-learn GaussianProcessRegressor, Telegram Bot API / Gmail SMTP for notifications
- OpenCelliD (optional, free key) for cell-tower positions - https://opencellid.org/
- Replaces the original plan's Spring Boot + PostgreSQL + Docker + Google Maps SDK + SMS gateway with FastAPI + SQLite + Leaflet/OpenStreetMap + email/Telegram, with the same features.
...
1. An Android phone (Android 10+) with a SIM card and a USB cable; install Android Studio (guided).
2. Which hardware is available: ESP32, NEO-6M GPS module? (Without it, use the simulator.)
3. Telegram bot token and/or Gmail app password for complaint notifications (guided).
4. Optional: OpenCelliD API key.
...
1. Ask for every item in section 9, one group at a time, with step-by-step guidance.
   ...
   - **Android field app:** guide the user to install Android Studio (latest stable, with the Android SDK), turn on Developer options and USB debugging on their phone, and connect it by USB. The app lives in `android/` (Kotlin + Jetpack Compose). It reads real cellular metrics through `TelephonyManager` (needs location and phone-state permissions), records GPS, stores readings in a local Room database while offline and syncs them when online. The phone reaches the backend on the same Wi-Fi (the PC's LAN IP, allowed through Windows Firewall) or through a Cloudflare quick tunnel (`winget install --id Cloudflare.cloudflared`, then `cloudflared tunnel --url http://localhost:8202`). Commit the source and put the built debug APK in `releases/`.
   - **ESP32 sensor node (only if the user has the hardware):** guide the Arduino IDE install, the ESP32 board package, flashing `firmware/esp32_node/` and entering Wi-Fi details. Without hardware, the built-in simulator replaces the node.
...
1. Install the Android app, walk around: readings appear live on the coverage map with Strong/Weak/Dead colours.
2. Stand in a dead zone -> the app suggests the nearest strong-signal spot; after the threshold a complaint is filed with evidence.
3. The engineer sees it in the operator desk and marks it resolved; new readings auto-verify or reopen it.
4. Turn on airplane mode, collect readings, turn it off -> readings sync.
...
- [ ] It runs on its own ports (8202 / 5202) while other products run on the same PC.
