...
def _seed(client, headers):
    ...
    for i in range(12):
        ...
        rows.append({"client_uuid": str(uuid.uuid4()), "ts": (base + timedelta(minutes=i)).isoformat(), "lat": 12.9716 + i * 0.0001,
                     "lon": 77.5946, "operator": "Airtel", "connection_type": "cellular", "connected": not dead, "probes_sent": 3,
...
def test_hexagons_and_heat(client, user_headers):
    ...
    fc = client.get("/api/coverage/hex?operator=Airtel", headers=user_headers).json()
    ...
    heat = client.get("/api/coverage/heat?operator=Airtel&mode=problems", headers=user_headers).json()
...
def test_hour_filter_uses_viewer_time_zone(client, user_headers):
    ...
    ist = client.get("/api/coverage/hex?operator=Airtel&hour_from=11&hour_to=12&tz_offset_min=330", headers=user_headers).json()
    assert ist["readings"] == 12
    utc = client.get("/api/coverage/hex?operator=Airtel&hour_from=11&hour_to=12&tz_offset_min=0", headers=user_headers).json()
    assert utc["readings"] == 0
    wrap = client.get("/api/coverage/hex?operator=Airtel&hour_from=22&hour_to=12&tz_offset_min=330", headers=user_headers).json()
...
def test_summary_points_and_nodes(client, user_headers):
    ...
    assert any(o["name"] == "Airtel" for o in s["operators"]) and s["total"] > 0 and s["bounds"]
    pts = client.get("/api/coverage/points?operator=Airtel&limit=5", headers=user_headers).json()
