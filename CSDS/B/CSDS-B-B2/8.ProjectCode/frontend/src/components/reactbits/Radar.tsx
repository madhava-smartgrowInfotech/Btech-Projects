...
interface RadarProps {
  ...
  lightMode?: boolean;
  /** SignalScout: draw a single still frame (used when the user prefers reduced motion). */
  paused?: boolean;
}
...
export default function Radar({
  ...
  useEffect(() => {
    ...
    // SignalScout: no WebGL (old device, blocked GPU) -> no background, the page still works.
    ...
    // SignalScout: render only while the hero is on screen.
    let visible = true;
    const observer = new IntersectionObserver(([entry]) => {
      visible = entry?.isIntersecting ?? true;
      if (visible && !paused && !animationFrameId) animationFrameId = requestAnimationFrame(update);
    });
    observer.observe(container);

    function update(time: number) {
      animationFrameId = paused || !visible ? 0 : requestAnimationFrame(update);
  ...
  }, [speed, scale, ringCount, spokeCount, ringThickness, spokeThickness, sweepSpeed, sweepWidth, sweepLobes, color, backgroundColor, falloff, brightness, enableMouseInteraction, mouseInfluence, lightMode, paused]);
