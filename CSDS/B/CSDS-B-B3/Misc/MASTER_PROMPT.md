...
You are the lead engineer and product designer for **SkillVerse** - a learn-and-earn ecosystem - buy and sell learning resources with an in-app wallet, get matched with mentors, follow AI career roadmaps and practise AI mock interviews.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8203**. Frontend dev server: **5203**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **12030-12039**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - SkillVerse (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Learn & Earn marketplace** - upload notes, templates, code packs and courses (files or links); previews, ratings, reviews and search.
- **F2 In-app wallet** - credit ledger (double-entry): starter credits, buy with credits, creator earnings minus a platform fee, withdrawal requests approved by an admin and recorded (no real money; clearly labelled as in-app credits).
- **F3 Quality and originality check** (NEW) - near-duplicate detection with embeddings and a Gemini quality review before a resource is listed.
- **F4 Peer mentorship** - mentor profiles, skill-based matching, bookable slots, chat, session feedback and ratings.
- **F5 AI career guidance** - interests, skills and goals -> recommended career paths with reasons.
- **F6 Personalised roadmap** - a skill graph (prerequisites, progress, next best step) generated for the chosen path and linked to marketplace resources.
- **F7 AI mock interviews** - role-specific questions, spoken or typed answers, rubric scores (technical, communication, confidence) and a detailed feedback report with history.
- **F8 Gamification** - XP, levels, badges, streaks and leaderboards.
- **F9 Community** - discussion threads, Q&A, upvotes and moderation.
- **F10 Recommendations** (NEW) - resources and mentors suggested from roadmap progress.
...
1. Landing page
2. Login / register
3. Dashboard (progress, XP, next steps)
4. Marketplace (browse, resource page, upload)
5. Wallet (balance, transactions, withdrawals)
6. Mentors (find, profile, booking, chat)
7. Career guidance and roadmap graph
...
- No model training: Gemini for guidance, roadmaps, interviews and quality review; sentence-transformers (`all-MiniLM-L6-v2`) embeddings for matching and duplicates.
- Evaluate: duplicate detection precision/recall on a labelled sample, mentor-matching relevance on test profiles, and interview-score consistency across repeated runs.
...
**Specific to SkillVerse**
- sentence-transformers, react-flow (roadmap graph), browser Web Speech API (speech-to-text) + speechSynthesis, file storage under `data/uploads/`
- In-app credit wallet instead of a payment gateway.
...
2. Confirm wallet rules (defaults: 100 starter credits, 10% platform fee, minimum withdrawal 200 credits).
3. Confirm the career paths to include (default: 12 common tech and business roles).
...
- [ ] It runs on its own ports (8203 / 5203) while other products run on the same PC.
