...
You are the lead engineer and product designer for **SignBridge** - real-time two-way communication between Indian Sign Language users and hearing people - signs to text and speech, speech to live subtitles, text to animated signs, working offline.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8205**. Frontend dev server: **5205**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **12050-12059**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - SignBridge (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Live sign recognition** - webcam -> MediaPipe Holistic landmarks -> signs with confidence: fingerspelling (A-Z, 1-9) and a word vocabulary.
- **F2 Sentence formation** - sliding-window segmentation turns a sign sequence into ISL gloss, then a grammatical sentence (Gemini when online, rule-based offline fallback) in English, Hindi or Telugu (NEW).
- **F3 Speech output** - the sentence is spoken aloud with offline text-to-speech.
- **F4 Speech -> live subtitles** - the hearing person speaks; offline speech recognition shows large live subtitles to the signer.
- **F5 Text/speech -> animated signs** (NEW) - words become an ISL sign sequence played by a 2-D skeleton avatar from recorded landmark sequences; unknown words are fingerspelled.
- **F6 Conversation mode** - split screen with a combined transcript of both sides; save and export.
- **F7 Teach-a-sign** (NEW) - record a few examples of a new sign; added instantly via landmark-embedding nearest neighbours, no retraining.
- **F8 Offline and latency meter** - everything runs offline except optional sentence polishing; FPS and latency shown.
- **F9 Model performance** - accuracy, confusion matrix, latency.
...
4. Sign-to-text practice view
5. Text/speech-to-sign avatar
6. Teach a sign
7. History
8. Model performance
...
- Kaggle GPU: landmark-sequence model (Transformer or BiLSTM) for word signs trained on the INCLUDE landmark data (start with about 50 everyday words), plus an MLP for fingerspelling on landmarks extracted from the ISL image set; export to ONNX for CPU inference.
- Report top-1 / top-5 accuracy, per-class F1, and end-to-end latency on this PC.
...
**Specific to SignBridge**
- MediaPipe Holistic / Hands, onnxruntime, faster-whisper or Vosk (offline speech recognition), pyttsx3 or Piper (offline text-to-speech), HTML canvas skeleton animation
- Replaces the original plan's Streamlit interface with React.
...
3. Confirm the starting vocabulary (default: 50 everyday words + alphabet + digits).
4. Optional: Gemini API key for sentence polishing - https://aistudio.google.com/apikey
...
1. Sign 'HELLO HOW YOU' -> 'Hello, how are you?' shown and spoken.
2. The hearing person replies by voice -> live subtitles appear for the signer, with internet turned off.
3. Type 'thank you' -> the avatar plays the signs.
4. Teach a new sign with 5 examples -> it is recognised immediately.
...
- [ ] It runs on its own ports (8205 / 5205) while other products run on the same PC.
