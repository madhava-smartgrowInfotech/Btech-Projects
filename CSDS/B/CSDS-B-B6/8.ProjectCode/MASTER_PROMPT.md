...
You are the lead engineer and product designer for **UniHealth** - one patient-controlled health record across hospitals - HL7 FHIR exchange, a master patient index, consent you can see and revoke, and an AI assistant that explains your records.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8206**. Frontend dev server: **5206**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **12060-12069**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - UniHealth (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Three simulated hospitals** - each an independent FHIR R4 server (own database, own patient IDs) loaded with different Synthea patients, with overlaps.
- **F2 Master Patient Index** - probabilistic linkage (name, date of birth, gender, phone, address with fuzzy matching) links one person's records across hospitals, with match confidence and a review queue for uncertain matches (NEW).
- **F3 Consent management** - the patient grants or revokes access by hospital, doctor, data category and time window; stored as FHIR Consent; hash-chained, tamper-evident consent ledger (NEW).
- **F4 Secure record retrieval** - a doctor's request -> consent check -> FHIR REST queries to other hospitals -> merged, de-duplicated timeline; JWT with scopes; full audit trail.
- **F5 Post-treatment sync** - hospitals push a FHIR summary bundle to the platform; originals stay with the hospital.
- **F6 Unified patient record** - timeline of encounters, conditions, medications, labs and allergies across hospitals; download as FHIR bundle or PDF.
- **F7 Health-risk prediction** - Random Forest models (heart disease, diabetes) using values from the record, with SHAP reasons.
- **F8 AI assistant** - explains lab reports in plain language (English, Hindi, Telugu), summarises treatment, answers questions grounded in the patient's own record.
- **F9 Reminders** - medication and follow-up reminders from MedicationRequest / CarePlan; in-app plus optional Telegram or email.
- **F10 Interoperability check** (NEW) - resources validated with fhir.resources and tested against the public HAPI FHIR R4 test server.
...
1. Landing page
2. Login (patient, doctor, records staff, admin)
3. Patient: unified timeline, consents, AI assistant, reminders
4. Doctor: patient search, access request, merged record, risk panel
5. Hospital console: sync status, FHIR resources
6. MPI review queue
7. Audit and consent ledger
8. Admin
9. Model performance
...
- Local CPU: Random Forest risk models (heart disease, diabetes) with ROC-AUC, accuracy and SHAP; MPI linkage evaluated on seeded duplicates with injected typos (precision / recall).
- Gemini for explanations and summaries.
...
**Specific to UniHealth**
- fhir.resources (FHIR R4 models and validation), rapidfuzz / jellyfish (record linkage)
- Each hospital FHIR server is a small FastAPI app on its own port with its own SQLite file, started by `run.bat`
- Public HAPI FHIR R4 test server for compatibility checks - https://hapi.fhir.org/baseR4
- Replaces the original plan's HAPI FHIR server + PostgreSQL + Flutter app with FastAPI FHIR servers + SQLite + a responsive web app; no Docker.
...
- [ ] It runs on its own ports (8206 / 5206) while other products run on the same PC.
