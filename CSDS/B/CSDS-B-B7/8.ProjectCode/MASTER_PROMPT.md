...
You are the lead engineer and product designer for **HospiSense** - predicts length of stay, bed and ICU demand, and recommends how to allocate beds, staff and equipment - with the reasons behind every number.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8207**. Frontend dev server: **5207**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **12070-12079**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - HospiSense (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Length-of-stay prediction** - at admission: expected days and a short/long class with SHAP reasons.
- **F2 Bed occupancy forecast** - next 7-14 days per facility and ward from admissions plus predicted stays, with uncertainty bands.
- **F3 ICU demand forecast** - expected ICU beds needed.
- **F4 Staff and equipment demand** - nurses per shift and key equipment from forecast load and ratios.
- **F5 Allocation optimiser** - OR-Tools recommends ward bed assignments, shift rosters and equipment distribution within capacity, and explains the trade-offs.
- **F6 Early warnings** (NEW) - predicted overcrowding days and ICU shortfalls.
- **F7 What-if simulator** (NEW) - admission surge, ward closure or staff absence -> recalculated plan.
- **F8 Decision dashboard** - KPIs, forecasts and recommendations to accept or modify.
- **F9 Model performance**.
...
1. Landing page
2. Login (admin, doctor, planner)
3. Dashboard (KPIs, alerts)
4. Admissions and LOS predictions (patient view with SHAP)
5. Occupancy and ICU forecasts
6. Allocation recommendations (beds, staff, equipment)
7. What-if simulator
8. Model performance
...
- Local CPU: XGBoost / LightGBM length-of-stay models on the Microsoft dataset (regression: MAE, R2) and the AV dataset (stay-band classification: accuracy, macro-F1); SHAP.
- Occupancy forecasting from daily census derived from admission and discharge dates (MAE / MAPE vs a naive baseline).
- Optimisation: OR-Tools; report objective value, constraint satisfaction and run time.
...
**Specific to HospiSense**
- XGBoost / LightGBM, SHAP, Google OR-Tools, statsmodels (optional)
- Replaces the original plan's Streamlit / Dash + PostgreSQL with React + SQLite.
...
2. Confirm the hospital structure (default: 5 facilities; wards General, Surgical, Maternity, Paediatric, ICU; nurse-to-patient ratios).
...
1. Admit a patient -> predicted 6.2 days, 'long stay', with SHAP reasons.
2. The forecast shows ICU demand exceeding capacity on day 5 -> early warning.
3. Run the optimiser -> bed moves, extra nurse shifts and ventilator transfer recommended; accept.
4. What-if: +30% admissions -> the plan recalculates.
...
- [ ] It runs on its own ports (8207 / 5207) while other products run on the same PC.
