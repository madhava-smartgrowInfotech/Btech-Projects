...
You are the lead engineer and product designer for **TalentTrack** - builds and proves job readiness - aptitude and coding practice with an automatic judge, contests, levels, AI mock interviews and resume analysis, connected to recruiters who shortlist by real skill.
...
11. **Fixed ports - other products may be running on this PC at the same time.** Backend API: **8209**. Frontend dev server: **5209**. Any extra local service (simulators, extra servers, test targets, worker UIs) uses a port in **12090-12099**. Put them in `.env` / `.env.example` and `vite.config.ts` (with `strictPort: true`); never use the defaults 8000 / 5173.
...
### 4.4 What you will build - TalentTrack (the latest, advanced, innovative system)

This combines the specification in 4.2 with the improvements below. Items marked **NEW** go beyond the specification.

- **F1 Aptitude practice** - question bank by topic and difficulty, timed tests, automatic evaluation, explanations.
- **F2 Coding practice and judge** - Monaco editor, problems with sample and hidden tests, local sandboxed runner (Python, C++, Java) with time and memory limits, verdicts (Accepted, Wrong Answer, TLE, Runtime Error, Compile Error).
- **F3 Technical MCQs** - programming, DBMS, OS, networks.
- **F4 Contests and leaderboards** - scheduled contests, live leaderboard (WebSocket), ratings.
- **F5 Level-based progression** - levels unlock by scores; the final level is an expert mock interview before the 'job-ready' badge.
- **F6 AI mock interviews** - voice or text, role-specific, rubric feedback.
- **F7 AI resume analysis** - PDF upload -> parsing -> ATS-style score, role match, missing skills, suggestions.
- **F8 Career chatbot** - answers questions using the candidate's own progress data.
- **F9 Recruiter module** - hiring drives with eligibility rules (scores, skills, level), ranked shortlist with skill evidence, invites, status pipeline.
- **F10 Readiness analytics** - cohort readiness, weak topics, drive outcomes for career-services teams.
- **F11 Code similarity check** (NEW) - flags near-identical submissions.
...
1. Landing page
2. Login (candidate, recruiter, career-services, expert)
3. Candidate dashboard (level, readiness score)
4. Aptitude tests
5. Coding problems and editor
6. Contests and leaderboard
7. Mock interview room and report
8. Resume analyser
9. Chatbot
10. Recruiter: drives, shortlist
11. Readiness analytics
12. Admin
...
- Local CPU: resume role classifier (TF-IDF + Linear SVM / Logistic Regression) trained on the Resume Dataset; report accuracy and macro-F1.
- Gemini for interviews, feedback, resume suggestions and chatbot; readiness score formula documented and shown in the product.
...
**Specific to TalentTrack**
- Monaco Editor (@monaco-editor/react), local code runner (subprocess per run in a temp folder, time limit, memory monitoring with psutil)
- g++ (WinLibs / MinGW-w64) and JDK 17 installed with guidance; pdfplumber (resume parsing); browser Web Speech API
- Replaces the original plan's Node.js / Express / MongoDB + Judge0 with FastAPI + SQLite + a local judge.
...
2. Install g++ and JDK 17 (guided).
3. Confirm the languages for the judge (default: Python, C++, Java) and the level structure.
...
- [ ] It runs on its own ports (8209 / 5209) while other products run on the same PC.
